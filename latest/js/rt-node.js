 ...
 * @deprecated
 */
export function initWaModulesFS(waPath) {
    return waNode.loadFS(waPath, rt.modules(), "mem", "space", "delay");
}
export function fsRuntime(waPath) {
    const modules = fsLoadRuntimeModules(waPath);
    return rt.linkRuntime(modules);
}
export function fsLoadRuntimeModules(waPath) {
    return waNode.fsLoadModules(waPath, rt.runtimeModulePaths());
}
